...
- [x] 6.1 feature flag(`BOT_PIONEER_SCHEDULER`) 코드 제거 — 프로덕션 배포 전 단계이므로 카나리/병행 운영 단계 없이 신규 경로를 유일 경로로 만든다 (구 6.1 "기본 true로 전환 + 스테이징/프로덕션 병행"은 배포 전이므로 폐기)
...
- [x] 6.5 (6.1과 통합됨)
- [x] 6.6 제거된 코드를 참조하던 테스트·진단 도구(`show-map` 등) 동작 재확인 — `go build ./...` + `go test ./...` 통과. `pioneer_snapshot_test.go` 삭제로 `TestPioneer_FailOpen_StoreErrorDoesNotBlockCrawl`도 함께 제거되었고, 신규 fail-close 규약은 `TestPioneerConsumer_FetchFailure_*` 시리즈가 보장한다
- [x] 6.7 `apps/api/fuguebot_pseudo.go` 상단 주석에 "부가 참고 자료: 정식 계약은 `openspec/specs/pioneer/spec.md`/`scheduler/spec.md`/`harvester/spec.md`" 명시 + Pioneer 호출부가 PioneerConsumer로 이미 전환되었음을 기록
