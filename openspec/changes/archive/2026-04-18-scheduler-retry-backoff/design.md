...
Archived change `scheduler-frontier-table`(`openspec/specs/scheduler` spec에 merge 완료)에서 `fetch_error_count`, `next_fetch_at`, `harvest_error_count`, `next_harvest_at` 컬럼과 `… < 5` 조건을 포함하는 partial index가 도입됐다. In-flight change `scheduler-claim-api`는 claim 프로토콜(`Dequeue`)과 성공 경로(`SetStatus`)를 정의한다. 하지만 실패 시 이 컬럼을 어떻게 갱신할지는 아직 정의되지 않았다.
...
```
delay  = 30s * 2^(error_count_after - 1)   // error_count_after는 이번 실패를 반영한 "후"의 값
jitter = uniform[-0.1 * delay, +0.1 * delay]   // uniform 분포 (정규분포 아님)
next_*_at = T_report + delay + jitter         // T_report는 워커가 보고 시점에 관측한 시각
```

- `error_count_after`는 **이번 실패를 반영한 후**의 값(1..5)을 공식에 넣는다. 즉 첫 실패(0 → 1, `error_count_after = 1`)면 `2^0 * 30s = 30s`, 두 번째(1 → 2, `error_count_after = 2`)면 `2^1 * 30s = 60s`, ... 네 번째(3 → 4)면 `2^3 * 30s = 240s`, 다섯 번째(4 → 5)면 `2^4 * 30s = 480s`가 반영된다. 다섯 번째 보고 UPDATE가 커밋되면 `fetch_error_count = 5`가 되어 partial index 조건(`< 5`)을 위반하므로 이후 claim 대상에서 제외된다(즉, 계산된 `next_fetch_at`은 관측에 사용되지 않는다).
- **최대 delay**: `30s * 2^4 = 480s (8분)`. `error_count_after <= 5`가 보장되므로 `int64` nanosecond overflow는 없다. cap이 명시적으로 5이므로 `2^n` 폭주 없음.
- **jitter 분포는 uniform**. Go의 `math/rand`로 `[-0.1, +0.1]` 균일 표집 후 delay에 곱한다(jitter는 non-cryptographic 용도이므로 `math/rand`로 충분). **정규분포(Normal) 아님**을 명시.
...
- 이유: (1) 단위 테스트에서 clock을 주입하여 결정적 검증이 가능, (2) 모든 워커가 Go `time.Now()` 기반으로 일관된 방식으로 시각을 산출하고, 각 호스트의 시계 차이(skew)만 고려하면 됨(DB 왕복으로 인한 추가 지연을 공식에 섞지 않음), (3) jitter PRNG가 DB 독립적이라 관찰/디버깅 편의.

**예시 의사 SQL (RecordFetchError, non-4xx):**
```sql
-- $1 = sha256(key)  (key는 normalized_url, 해싱은 호출부 Go 측 책임)
-- $2 = T_report + delay + jitter  (Go가 계산)
...
```sql
-- 파라미터는 $1 = sha256(key) 하나뿐. next_fetch_at을 UPDATE하지 않으므로 $2 없음.
-- next_fetch_at의 기존 값은 일반적으로 claim 시 설정된 lease marker(= T_claim + 10분)
-- 이며, 해당 row가 partial index에서 제외되므로 더 이상 관측에 사용되지 않는다.
-- NOT NULL 제약은 기존 값으로 이미 충족.
UPDATE pioneer_frontier
...
정책 표는 Pioneer 측(`fetch_error_count`, `next_fetch_at`) 기준이며, Harvester 측(`harvest_error_count`, `next_harvest_at`)에도 동일한 정책을 대응 컬럼에 적용한다.

| errorKind | Pioneer 측 정책 | Harvester 측 정책 |
|-----------|----------------|------------------|
| `http_4xx` | 즉시 dead. `fetch_error_count = 5`로 set. 공식 **미적용**. | 즉시 dead. `harvest_error_count = 5`로 set. 공식 **미적용**. |
| `http_5xx` | 공식 적용, `fetch_error_count += 1`. | 공식 적용, `harvest_error_count += 1`. |
| `network` | 공식 적용, `fetch_error_count += 1`. | 공식 적용, `harvest_error_count += 1`. |
| `timeout` | 공식 적용, `fetch_error_count += 1`. | 공식 적용, `harvest_error_count += 1`. |
...
- [jitter 구현 편차] → 명세는 "±10% uniform 분포"만 요구한다. 초기 구현은 `math/rand`를 채택하되, 테스트 주입 용이성을 위해 jitter 헬퍼 시그니처를 `func(delay time.Duration) time.Duration` 추상으로 유지한다. 구체 PRNG는 **scheduler 구현체 생성자 레벨에서 캡슐화**하는 것을 권장한다(헬퍼 내부 static PRNG도 허용). 테스트에서는 헬퍼를 stub으로 교체하거나 시드 주입된 구현으로 대체해 경계값을 검증한다.
