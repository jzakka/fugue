...
메인 화면에서 콘텐츠를 발견할 수 있는 유일한 방법이 피드 스크롤과 미디어 타입/태그 필터뿐이다. 핀이 쌓일수록 원하는 콘텐츠를 다시 찾기 어렵고, 새로운 크리에이터나 보드를 탐색할 수단이 없다. 큐레이션 플랫폼의 핵심인 "검색을 통한 발견"이 빠져 있다.
...
_(기존 도메인 스펙의 시나리오 추가는 없으나, pin/board/profile 도메인의 엔티티를 검색 대상으로 사용한다)_
...
- **API**: `GET /api/search?q=&type=all|pins|creators|boards&tag_ids=&limit=&offset=` 신규 엔드포인트
