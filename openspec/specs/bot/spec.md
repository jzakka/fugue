...
`bot` capability는 Fugue의 콘텐츠 발견·추출 파이프라인을 정의한다. Pioneer는 사이트 그래프를 BFS로 탐색하며 페이지 템플릿 패턴 단위로 노드/엣지를 저장하고 노드 타입별 파싱 스크립트를 보유한다. Harvester는 그 그래프를 따라 실제 콘텐츠를 추출해 핀으로 적재한다. 본 스펙은 이 두 행위자의 외부 관찰 가능 동작(그래프 정규화 규칙, 스크립트 검증·저장, 페이지 fetch·재시도, 스냅샷 저장)을 행위 계약으로 기술한다.
## Requirements
