...
8. **커밋 + PR**: 이 사이클의 모든 변경을 커밋하고 PR을 올린다. main 머지는 절대 하지 않는다.
   ...
   - 현재 브랜치에서 분기 브랜치를 만든다: `loop/design/<change-name>`. 이미 존재하면 `-YYYYMMDD-HHMM` 접미사를 붙인다.
   - 커밋 prefix는 변경 성격에 맞춰 `feat:` / `fix:` / `chore:` / `refactor:` 중 하나. 메시지 본문에는 방금 `.fugue/decision-log.md`에 추가한 1~3줄을 그대로 인용한다.
   - `git push -u origin <branch>` 후 `gh pr create --base main`으로 PR을 만든다. PR 본문에는 evidence(파일/라인), `DESIGN.md` 인용, 변경 범위, 사용자 영향, 롤백 절차, `Auto-generated by Fugue 디자인 트랙 루프 (cycle <iteration>)` 표기를 포함한다.
